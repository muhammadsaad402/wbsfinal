...
export default function ReviewedOrdersCard() {
  ...
  const getOrders = () => {
    axios
      ...
      .then((response) => {
        setOrders(response.data.data.review);
  ...
  return (
    <div className={styles.parent_row}>
      {orders?.some((cardData) => cardData.status === "review") && (
        <h1>Orders Under Review</h1>
      )}
        ...
        {orders?.map((cardData) => (
          <div key={cardData?.id}>
            ...
            >
              ...
              <div className={styles.card_sec2}>
                <h6 style={{ marginBottom: "5px" }}>Occasion</h6>
            ...
            <Link
              ...
              href={{
                pathname: "/review_order",
                query: { order_id: cardData?.id, order_video: cardData?.video },
              }}
            >
              Proceed to review
            </Link>
