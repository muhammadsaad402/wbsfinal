...
export default function RejectedOrdersCard() {
  ...
  const getOrders = () => {
    axios
      ...
      .then((response) => {
        setOrders(response.data.data.reject_orders);
  ...
  return (
    <div className={styles.parent_row}>
      {/* {orders && <h1>Rejected Orders</h1>} */}
      {orders?.some((cardData) => cardData.status === "reject") && (
        <h1>Rejected Orders</h1>
      )}
      <div className={styles.row}>
        {orders?.map((cardData) => (
          <div key={cardData?.id}>
            {/* {(console.log(orders), "resg")} */}
